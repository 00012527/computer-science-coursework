...
class Snake(object):
    ...
    def turn(self, point):
        if self.length > 1 and (point[0] * -1, point[1] * -1) == slef.direction:
            return
        else:
            self.direction = point

    def move(self):
        current = self.get_head_position()  # to find head
        x, y = self.direction
    ...
    def reset(self):
        self.length = 1
        self.positions = [((WIDTH / 2), (HEIGHT / 2))]
        self.direction = random.choice([UP, DOWN, LEFT, RIGHT])  # these are for restarting the game
